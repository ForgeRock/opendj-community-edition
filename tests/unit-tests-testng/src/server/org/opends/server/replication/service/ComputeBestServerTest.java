...
public class ComputeBestServerTest extends ReplicationTestCase
{
  ...
  private ServerState newServerState(ChangeNumber... cns)
  {
    ServerState result = new ServerState();
    for (ChangeNumber cn : cns)
    {
      result.update(cn);
    }
    return result;
  ...
  /**
   * Test with one replication server, nobody has a ChangeNumber (simulates) very first
   * connection.
   */
  @Test
  ...
  public void testNullCNDS() throws Exception
  {
    ...
    ServerState mySt = newServerState();
    ServerState aState = newServerState(new ChangeNumber(0, 0, myId1));
  ...
  /**
   * Test with one replication server, only ds server has a non null ChangeNumber for ds
  ...
  public void testNullCNRS() throws Exception
  {
    ...
    ServerState mySt = newServerState(new ChangeNumber(1, 0, myId1));
    ServerState aState = newServerState();
  ...
  public void test1ServerUp() throws Exception
  {
    ...
    ServerState mySt = newServerState(new ChangeNumber(1, 0, myId1));
    ServerState aState = newServerState(new ChangeNumber(1, 0, myId1));
  ...
  public void test2ServersUp() throws Exception
  {
    ...
    ServerState mySt = newServerState(new ChangeNumber(1, 0, myId1));
    ServerState aState1 = newServerState(new ChangeNumber(1, 0, myId1));
    ServerState aState2 = newServerState(new ChangeNumber(2, 0, myId1));
  ...
  public void test3ServersUp() throws Exception
  {
    ...
    ServerState aState1 = newServerState(new ChangeNumber(1, 0, myId1));
    ServerState aState2 = newServerState(new ChangeNumber(2, 0, myId1));
    ServerState aState3 = newServerState(new ChangeNumber(3, 0, myId1));
  ...
  public void test2ServersUpToDateAnd1EvenMoreUpToDate() throws Exception
  {
    ...
    ServerState mySt = newServerState(new ChangeNumber(1, 0, myId1));
    ServerState aState1 = newServerState(new ChangeNumber(1, 0, myId1));
    ServerState aState2 = newServerState(new ChangeNumber(1, 0, myId1));
    ServerState aState3 = newServerState(new ChangeNumber(2, 0, myId1));
      ...
      throws Exception
  {
    ...
    // Create my state
    ServerState mySt = newServerState(
        new ChangeNumber(4, 0, myId1),
        new ChangeNumber(2, 0, myId2),  // myId2 should not be used inside algo (same for the other ServerStates)
        new ChangeNumber(3, 0, myId3)); // myId3 should not be used inside algo (same for the other ServerStates)

    // State for server 1
    ServerState aState1 = newServerState(
        new ChangeNumber(looser1T1, 0, myId1),
        new ChangeNumber(looser1T2, 0, myId2),
        new ChangeNumber(looser1T3, 0, myId3));
    if (looser1IsLocal)
      ReplicationServer.onlyForTestsAddlocalReplicationServer(LOOSER1);

    // State for server 2
    ServerState aState2 = newServerState(
        new ChangeNumber(winnerT1, 0, myId1),
        new ChangeNumber(winnerT2, 0, myId2),
        new ChangeNumber(winnerT3, 0, myId3));
    if (winnerIsLocal)
      ReplicationServer.onlyForTestsAddlocalReplicationServer(WINNER);

    // State for server 3
    ServerState aState3 = newServerState(
        new ChangeNumber(looser2T1, 0, myId1),
        new ChangeNumber(looser2T2, 0, myId2),
        new ChangeNumber(looser2T3, 0, myId3));
    if (looser2IsLocal)
      ...
      throws Exception
  {
    ...
    // Create my state
    ServerState mySt = newServerState(new ChangeNumber(4, 0, myId1));

    // State for server 1
    ServerState aState1 = newServerState(new ChangeNumber(looser1T1, 0, myId1));
    if (looser1IsLocal)
      ReplicationServer.onlyForTestsAddlocalReplicationServer(LOOSER1);

    // State for server 2
    ServerState aState2 = newServerState(new ChangeNumber(winnerT1, 0, myId1));
    if (winnerIsLocal)
      ReplicationServer.onlyForTestsAddlocalReplicationServer(WINNER);

    // State for server 3
    ServerState aState3 = newServerState(new ChangeNumber(looser2T1, 0, myId1));
    if (looser2IsLocal)
      ReplicationServer.onlyForTestsAddlocalReplicationServer(LOOSER2);
